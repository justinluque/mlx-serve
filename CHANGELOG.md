...
- **Convert your own**, at 4-bit, 8-bit, or the `mixed_3_8` policy (`tests/convert_ideogram4.py --precision mixed`). The converter keeps modulation and conditioning projections at 8-bit; `--bulk-bits` / `--sensitive-bits` / `--te-bits` override the tier.
