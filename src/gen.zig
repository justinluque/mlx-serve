...
// is active. Auto-downloaded into ~/.mlx-serve/models; "missing" is never an
// error — it just means the cheaper linear-projection preview is used instead
// of a real decode, and generation is never blocked.
// ════════════════════════════════════════════════════════════════════════
...
fn taesdDecoderFor(allocator: std.mem.Allocator, engine: *ImageEngine) ?latent_preview.PreviewDecoder {
    // `handleImage` carries no `std.Io`; this runs once per process on the
    // inference thread (the sole mlx caller), so a local one is the idiom here.
...
pub fn handleImage(allocator: std.mem.Allocator, conn: *Conn, body: []const u8, engine: *ImageEngine) !void {
    ...
    // Live per-step "ghost image" previews (see latent_preview.zig): TAESD
    // when its decoder is present, otherwise a cheap linear latent→RGB
    // projection. Opt-in and only meaningful with streaming — `preview`
    // without `stream` is a no-op.
    const want_preview = want_stream and sse.bodyWantsTrue(body, "preview");
    // Testing knobs for the Image pane's "Latent RGB" / "TAESD" checkboxes —
    // independently disable either preview mechanism so the two can be
    // exercised (and compared/debugged) in isolation. Both default true
    // (normal operation: prefer taesd, fall back to linear).
    ...
    var sctx = sse.StreamCtx{
        ...
        .taesd = if (want_preview and preview_taesd) taesdDecoderFor(allocator, engine) else null,
