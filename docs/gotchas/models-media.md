...
## A LoKr's alpha divisor is a rank the file usually does not carry (`lora.lokrScale`, 2026-09-06)

An Ideogram 4 LoKr rendered a **pure white** frame at every prompt and seed. The Kronecker math was already pinned (`deltaLokr`'s hand-derived identity test), the orientation was right, and every shape lined up — which is itself the diagnosis: a wrong `q·c` raises in `mlx_reshape` and a wrong `p·r` raises in the `mlx_add` against the base output, and an MLX raise ABORTS. **A LoKr frame that renders at all has a correctly-shaped delta, so a broken one can only be a MAGNITUDE bug.** That single inference skipped the whole search.

The magnitude came from `alpha / w1.shape[0]`. w1 is `[out_l, in_m]` from LyCORIS' `factorization(dim, factor)`, so its row count is a factorization LEG, not a rank — 8 at `factor 8`, 48 for a 3072-wide output at `factor -1`. The tell was available without a reference implementation: **the divisor VARIES per module** (attention and FFN in one file factor differently), so one alpha produced several different scales, which no rank ever does. The adapter's every module carried `alpha = 1e10` — LyCORIS' full-matrix recipe, `network_dim = network_alpha`, i.e. the `alpha = lora_dim` DEFAULT written out, whose true scale is 1.0. `1e10 / 8 = 1.25e9` went into all 204 modules; measured `rms(kron(w1,w2))` is 0.0013–0.0032, so the delta shipped at ~1.7–4.0 MILLION rms against a DiT linear whose own weights are ~0.02.

The fix is a precedence, not a better divisor. **Alpha is HALF of a pair and cannot scale anything alone.** The other half — LyCORIS' `lora_dim` — survives into a checkpoint only as the INNER dimension of a factored `lokr_*_a`/`lokr_*_b` pair, and the fold destroys it, so `loadFile` reads it in the fold loop or never. A FULL (unfactorized) pack carries no rank at all; there the dim lives in `__metadata__` (`ss_network_dim`, PEFT `r`) or nowhere. So: a complete per-module pair (alpha + derived rank) wins, the file's own declared pair is next, and 1.0 is the floor — and that floor is not a shrug, it is the true scale of every adapter that never overrode LyCORIS' `alpha = lora_dim` default. The chosen rung and its reason are logged once per file (`[lora] <file>: LoKr scale … — …`), because a scale this load-bearing must never be invisible.

**Two adjacent silences fell out of the same review.** `parseKey` dropped every suffix it did not recognize, so a module shipping `scalar` (LyCORIS `use_scalar` — with it BOTH factors are kaiming-initialized and the trained scalar carries the entire magnitude), `dora_scale`, or `lokr_t2` still counted as "complete" on w1+w2 alone and got a delta computed as if that tensor were 1 — an independent second route to the same white frame. They are now CLASSIFIED (`Role.unsupported`) so the module is refused BY NAME, the same rule as a missing factor. And `Ref.kind` was copied only by `Stack.findAll`: minimax_h3 and ltx_video inline their own per-file attribution and both defaulted to `.lora`, so a LoKr on either ran `(x@A)@B` over Kronecker factors. The defect is the CALLER SHAPE — a `Ref` literal assembled field by field — so the guard scans every LoRA-consuming file for it rather than pinning the two known sites.
